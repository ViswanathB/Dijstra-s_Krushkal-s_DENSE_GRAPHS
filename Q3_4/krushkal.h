...
int alledges_heap_next_element;
...
void DFS(int v) {
	...
	mast[v].color = 2;//the actually doesn't make a difference since this is acyclic graph 
...
void krushkals_max_bandwidth(struct graph *g, int src, int dest) {
	...
	if ( src ==200 && dest ==2000) {
	} else {
	printf("\n The maximum flow possible from:%d -> to:%d is %d",src,dest, mast[dest].flow);
		printf("\n The running time is %f sec",(double)(end-begin)/CLOCKS_PER_SEC);
		...
		while(src!=dest) {
			...
			printf(" ->((%d))-> %d\t",maximum(mast[dest].flow,mast[i].flow),dest);
